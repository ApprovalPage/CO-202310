(function (cjs, an) {

var p; // shortcut to reference prototypes
var lib={};var ss={};var img={};
lib.ssMetadata = [];


(lib.AnMovieClip = function(){
	this.actionFrames = [];
	this.ignorePause = false;
	this.gotoAndPlay = function(positionOrLabel){
		cjs.MovieClip.prototype.gotoAndPlay.call(this,positionOrLabel);
	}
	this.play = function(){
		cjs.MovieClip.prototype.play.call(this);
	}
	this.gotoAndStop = function(positionOrLabel){
		cjs.MovieClip.prototype.gotoAndStop.call(this,positionOrLabel);
	}
	this.stop = function(){
		cjs.MovieClip.prototype.stop.call(this);
	}
}).prototype = p = new cjs.MovieClip();
// symbols:
// helper functions:

function mc_symbol_clone() {
	var clone = this._cloneProps(new this.constructor(this.mode, this.startPosition, this.loop, this.reversed));
	clone.gotoAndStop(this.currentFrame);
	clone.paused = this.paused;
	clone.framerate = this.framerate;
	return clone;
}

function getMCSymbolPrototype(symbol, nominalBounds, frameBounds) {
	var prototype = cjs.extend(symbol, cjs.MovieClip);
	prototype.clone = mc_symbol_clone;
	prototype.nominalBounds = nominalBounds;
	prototype.frameBounds = frameBounds;
	return prototype;
	}


(lib.HEAD2 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#FFFFFF").s().p("AgOAOQgGgGAAgIQAAgHAGgHQAHgFAHgBQAIABAHAFQAGAHAAAHQAAAIgGAGQgHAGgIAAQgHAAgHgGg");
	this.shape.setTransform(140.825,33.05);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("#FFFFFF").s().p("AgwAnIAOgYQAGAGALAEQALAFAJAAQAOAAAAgIQAAgEgKgCIgVgEQgegHAAgZQAAgOALgKQANgLAVAAQAYAAAUAOIgOAYQgMgLgSAAQgNAAAAAHQAAAEAKACIAUAEQAfAHAAAZQAAAQgNAJQgNAKgWAAQgfAAgSgRg");
	this.shape_1.setTransform(132.5,29.525);

	this.shape_2 = new cjs.Shape();
	this.shape_2.graphics.f("#FFFFFF").s().p("AgmApQgQgQAAgZQAAgWAPgQQARgRAXAAQAYAAAPAQQAPARAAAaIAAAHIhKAAQAEASAVAAQAHAAAHgDQAIgCAEgEIAPAWQgQAOgcAAQgZAAgQgPgAAXgLQgCgQgUAAQgSAAgDAQIArAAIAAAAg");
	this.shape_2.setTransform(121.275,29.525);

	this.shape_3 = new cjs.Shape();
	this.shape_3.graphics.f("#FFFFFF").s().p("AghAoQgRgPAAgZQAAgYARgPQAQgQAYAAQAeAAAOAUIgWAUQgHgKgNAAQgKAAgHAHQgHAHAAALQAAAMAHAHQAHAHAKAAQAMAAAIgKIAWAUQgOAUgeAAQgYAAgQgQg");
	this.shape_3.setTransform(109.825,29.525);

	this.shape_4 = new cjs.Shape();
	this.shape_4.graphics.f("#FFFFFF").s().p("AgQBNIAAhpIAhAAIAABpgAgNgrQgFgGAAgIQAAgIAFgGQAFgFAIAAQAIAAAGAFQAFAGAAAIQAAAIgFAGQgGAGgIAAQgIAAgFgGg");
	this.shape_4.setTransform(101.3,27.125);

	this.shape_5 = new cjs.Shape();
	this.shape_5.graphics.f("#FFFFFF").s().p("AgRA1IgqhpIAkAAIAXBCIAYhCIAkAAIgqBpg");
	this.shape_5.setTransform(92.625,29.55);

	this.shape_6 = new cjs.Shape();
	this.shape_6.graphics.f("#FFFFFF").s().p("AghA2IAAhpIAhAAIAAAOQAGgIAJgEQAJgEAKgBIAAAhIgJgBQgHAAgHADQgHACgEAFIAABCg");
	this.shape_6.setTransform(83.2,29.4);

	this.shape_7 = new cjs.Shape();
	this.shape_7.graphics.f("#FFFFFF").s().p("AgmApQgQgQAAgZQAAgWAPgQQARgRAXAAQAYAAAPAQQAPARAAAaIAAAHIhKAAQAEASAVAAQAHAAAHgDQAIgCAEgEIAPAWQgQAOgcAAQgZAAgQgPgAAXgLQgCgQgUAAQgSAAgDAQIArAAIAAAAg");
	this.shape_7.setTransform(72.625,29.525);

	this.shape_8 = new cjs.Shape();
	this.shape_8.graphics.f("#FFFFFF").s().p("AgwAnIAOgYQAGAGALAEQALAFAJAAQANAAAAgIQAAgEgJgCIgUgEQgfgHAAgZQAAgOAMgKQAMgLAVAAQAYAAATAOIgNAYQgMgLgSAAQgMAAgBAHQAAAEAKACIAUAEQAfAHAAAZQAAAQgNAJQgNAKgWAAQgfAAgSgRg");
	this.shape_8.setTransform(61.2,29.525);

	this.shape_9 = new cjs.Shape();
	this.shape_9.graphics.f("#FFFFFF").s().p("AgQBJIAAiRIAhAAIAACRg");
	this.shape_9.setTransform(47.425,27.525);

	this.shape_10 = new cjs.Shape();
	this.shape_10.graphics.f("#FFFFFF").s().p("AgnAvQgMgKAAgQQAAgSAMgIQAKgHAPAAQAWAAAKAMIAAgKQAAgIgGgEQgGgEgJAAQgQAAgOALIgMgXQATgRAcAAQAyAAAAArIAABBIgiAAIAAgKQgLANgVAAQgOAAgLgJgAgMAMQgFADAAAGQAAAGAFAEQAFADAHAAQAMAAAGgIIAAgJQgFgIgNAAQgHAAgFADg");
	this.shape_10.setTransform(38.125,29.525);

	this.shape_11 = new cjs.Shape();
	this.shape_11.graphics.f("#FFFFFF").s().p("AgTAmIAAgvIgSAAIAAgeIASAAIAAgdIAhAAIAAAdIAWAAIAAAeIgWAAIAAAlQAAALAJAAQAGAAACgCIAHAZQgHAHgSAAQggAAAAgfg");
	this.shape_11.setTransform(28.45,28.2);

	this.shape_12 = new cjs.Shape();
	this.shape_12.graphics.f("#FFFFFF").s().p("AATA2IAAg8QAAgRgRgBQgMAAgIALIAABDIgiAAIAAhpIAiAAIAAAOQANgQAXgBQARAAAJAKQAJAJAAAQIAABJg");
	this.shape_12.setTransform(18.025,29.4);

	this.shape_13 = new cjs.Shape();
	this.shape_13.graphics.f("#FFFFFF").s().p("AgmApQgQgQAAgZQAAgWAPgQQARgRAXAAQAYAAAPAQQAPARAAAaIAAAHIhKAAQAEASAVAAQAHAAAHgDQAIgCAEgEIAPAWQgQAOgcAAQgZAAgQgPgAAXgLQgCgQgUAAQgSAAgDAQIArAAIAAAAg");
	this.shape_13.setTransform(5.525,29.525);

	this.shape_14 = new cjs.Shape();
	this.shape_14.graphics.f("#FFFFFF").s().p("AgpA8QgOgPAAgaQAAgZAOgPQAMgPAVAAQASAAANAPIAAg1IAhAAIAACSIghAAIAAgNQgOAPgRAAQgVAAgMgOgAgOAAQgGAHAAAMQAAALAGAIQAHAGAJABQANAAAIgKIAAghQgIgIgNAAQgJAAgHAGg");
	this.shape_14.setTransform(-7.4,27.65);

	this.shape_15 = new cjs.Shape();
	this.shape_15.graphics.f("#FFFFFF").s().p("AATA2IAAg8QAAgRgRgBQgMAAgIALIAABDIgiAAIAAhpIAiAAIAAAOQANgQAXgBQARAAAJAKQAJAJAAAQIAABJg");
	this.shape_15.setTransform(-25.625,29.4);

	this.shape_16 = new cjs.Shape();
	this.shape_16.graphics.f("#FFFFFF").s().p("AgpAnQgQgQAAgXQAAgWAQgQQAQgRAZAAQAaAAARARQAPAQAAAWQAAAXgPAQQgRARgaAAQgZAAgQgRgAgQgRQgGAHAAAKQAAALAGAHQAGAIAKAAQALAAAGgIQAGgHAAgLQAAgKgGgHQgGgIgLAAQgKAAgGAIg");
	this.shape_16.setTransform(-38.475,29.525);

	this.shape_17 = new cjs.Shape();
	this.shape_17.graphics.f("#FFFFFF").s().p("AgpA8QgOgPAAgaQAAgZAOgPQAMgPAVAAQASAAANAPIAAg1IAhAAIAACSIghAAIAAgNQgOAPgRAAQgVAAgMgOgAgOAAQgGAHgBAMQABALAGAIQAHAGAJABQANAAAIgKIAAghQgIgIgNAAQgJAAgHAGg");
	this.shape_17.setTransform(-57.25,27.65);

	this.shape_18 = new cjs.Shape();
	this.shape_18.graphics.f("#FFFFFF").s().p("AATA2IAAg8QAAgRgRgBQgMAAgIALIAABDIgiAAIAAhpIAiAAIAAAOQANgQAXgBQARAAAJAKQAJAJAAAQIAABJg");
	this.shape_18.setTransform(-69.875,29.4);

	this.shape_19 = new cjs.Shape();
	this.shape_19.graphics.f("#FFFFFF").s().p("AgmApQgQgQAAgZQAAgWAPgQQARgRAXAAQAYAAAPAQQAPARAAAaIAAAHIhKAAQAEASAVAAQAHAAAHgDQAIgCAEgEIAPAWQgQAOgcAAQgZAAgQgPgAAXgLQgCgQgUAAQgSAAgDAQIArAAIAAAAg");
	this.shape_19.setTransform(-82.375,29.525);

	this.shape_20 = new cjs.Shape();
	this.shape_20.graphics.f("#FFFFFF").s().p("Ag3BLIAAiSIAhAAIAAAMQANgPASAAQAVAAANAPQANAPAAAaQAAAZgNAPQgNAPgVAAQgSAAgNgPIAAA1gAgWgiIAAAgQAIAJANAAQAJAAAHgHQAGgGAAgMQAAgMgGgHQgHgHgJAAQgNAAgIAKg");
	this.shape_20.setTransform(-94.55,31.425);

	this.shape_21 = new cjs.Shape();
	this.shape_21.graphics.f("#FFFFFF").s().p("AgwAnIAOgYQAGAGALAEQAMAFAIAAQAOAAgBgIQAAgEgJgCIgUgEQgfgHAAgZQAAgOAMgKQAMgLAUAAQAaAAASAOIgMAYQgMgLgUAAQgMAAAAAHQAAAEAKACIAUAEQAfAHAAAZQAAAQgNAJQgNAKgWAAQgfAAgSgRg");
	this.shape_21.setTransform(-106.75,29.525);

	this.shape_22 = new cjs.Shape();
	this.shape_22.graphics.f("#FFFFFF").s().p("AgpAnQgQgQAAgXQAAgWAQgQQAQgRAZAAQAaAAARARQAPAQAAAWQAAAXgPAQQgRARgaAAQgZAAgQgRgAgQgRQgGAHAAAKQAAALAGAHQAGAIAKAAQALAAAGgIQAGgHAAgLQAAgKgGgHQgGgIgLAAQgKAAgGAIg");
	this.shape_22.setTransform(-123.925,29.525);

	this.shape_23 = new cjs.Shape();
	this.shape_23.graphics.f("#FFFFFF").s().p("AgTAmIAAgvIgSAAIAAgeIASAAIAAgdIAhAAIAAAdIAVAAIAAAeIgVAAIAAAlQAAALAJAAQAHAAACgCIAGAZQgHAHgSAAQggAAAAgfg");
	this.shape_23.setTransform(-134.2,28.2);

	this.shape_24 = new cjs.Shape();
	this.shape_24.graphics.f("#FFFFFF").s().p("AgaA2IAAhpIATAAIAAAQQAOgSAUAAIAAAUIgHgBQgHAAgJAFQgHAEgEAGIAABJg");
	this.shape_24.setTransform(82.875,6.975);

	this.shape_25 = new cjs.Shape();
	this.shape_25.graphics.f("#FFFFFF").s().p("AghAvQgMgKAAgQQAAgRAMgJQALgIAOAAQAWAAAMANIAAgRQAAgJgHgGQgHgGgMAAQgRAAgOAOIgIgNQARgSAZAAQATAAALAJQANAJAAATIAABHIgUAAIAAgLQgNAOgVAAQgOAAgLgJgAgSAGQgHAGAAAJQAAAJAHAGQAHAGALAAQARAAAJgMIAAgSQgJgMgRAAQgLAAgHAGg");
	this.shape_25.setTransform(71.875,7.075);

	this.shape_26 = new cjs.Shape();
	this.shape_26.graphics.f("#FFFFFF").s().p("AgkAoQgPgPAAgZQAAgWAPgQQAPgRAWAAQAYAAAOARQANAQAAAYIAAAFIhTAAQACAOAJAJQAKAKAPAAQATAAAMgNIAKANQgRAQgaAAQgXAAgQgQgAAhgHQAAgMgIgJQgIgKgQAAQgOAAgJAKQgHAJgCAMIBAAAIAAAAg");
	this.shape_26.setTransform(60.3,7.075);

	this.shape_27 = new cjs.Shape();
	this.shape_27.graphics.f("#FFFFFF").s().p("AgrBJIADgSQAEACAFAAQALAAAFgLIAFgNIgshrIAWAAIAgBSIAhhSIAWAAIg1B9QgIAYgYAAQgHAAgGgCg");
	this.shape_27.setTransform(48.6,9.25);

	this.shape_28 = new cjs.Shape();
	this.shape_28.graphics.f("#FFFFFF").s().p("AghAvQgMgKAAgQQAAgRAMgJQALgIAOAAQAWAAAMANIAAgRQAAgJgHgGQgHgGgMAAQgRAAgOAOIgIgNQARgSAZAAQATAAALAJQANAJAAATIAABHIgUAAIAAgLQgNAOgVAAQgOAAgLgJgAgSAGQgHAGAAAJQAAAJAHAGQAHAGALAAQARAAAJgMIAAgSQgJgMgRAAQgLAAgHAGg");
	this.shape_28.setTransform(30.375,7.075);

	this.shape_29 = new cjs.Shape();
	this.shape_29.graphics.f("#FFFFFF").s().p("AgrA0QgNgVAAgfQAAgeANgUQAQgZAbAAQAdAAAPAZQAOAUAAAeQAAAfgOAVQgPAYgdAAQgbAAgQgYgAgagmQgIAPAAAXQAAAYAIAPQAJARARAAQASAAAJgRQAIgPAAgYQAAgXgIgPQgJgRgSAAQgRAAgJARg");
	this.shape_29.setTransform(13.3,5.075);

	this.shape_30 = new cjs.Shape();
	this.shape_30.graphics.f("#FFFFFF").s().p("AgrA0QgOgVAAgfQAAgeAOgUQAPgZAcAAQAcAAAQAZQANAUAAAeQAAAfgNAVQgQAYgcAAQgcAAgPgYgAgagmQgIAPAAAXQAAAYAIAPQAJARARAAQASAAAJgRQAIgPAAgYQAAgXgIgPQgJgRgSAAQgRAAgJARg");
	this.shape_30.setTransform(1.1,5.075);

	this.shape_31 = new cjs.Shape();
	this.shape_31.graphics.f("#FFFFFF").s().p("Ag0AzIAMgPQAPATAZAAQANAAAJgIQAKgJgBgMQABgNgKgHQgIgIgOAAQgSABgOAMIgQgFIAAhQIBcAAIAAATIhGAAIAAAvQANgNATABQAUAAANAMQAPAMAAAVQAAAXgPANQgQAOgXAAQgiAAgSgYg");
	this.shape_31.setTransform(-11.15,5.2);

	this.shape_32 = new cjs.Shape();
	this.shape_32.graphics.f("#FFFFFF").s().p("AgNATQAFgDAEgGQAEgGABgEIgDAAQgFAAgDgDQgDgEAAgFQAAgGADgEQAEgEAGAAQAFAAAFAFQAEAFAAAIQAAAVgSAOg");
	this.shape_32.setTransform(-20.275,12.575);

	this.shape_33 = new cjs.Shape();
	this.shape_33.graphics.f("#FFFFFF").s().p("AAHBJIAAh1IgWAZIgNgNIAmgoIATAAIAACRg");
	this.shape_33.setTransform(-29.825,5.075);

	this.shape_34 = new cjs.Shape();
	this.shape_34.graphics.f("#FFFFFF").s().p("AgFBfIAAgTQgggDgTgUIANgRQAQARAWADIAAgwQgWgFgJgHQgPgKAAgSQAAgRANgMQAMgMAVgBIAAgVIAPAAIAAAVQAaADARARIgNAQQgMgNgSgDIAAArQAWAGAKAHQAPALAAATQAAARgLALQgNAOgXABIAAAUgAAKA4QAMgCAHgGQAFgGAAgIQAAgIgHgGQgFgEgMgEgAgWgxQgGAHAAAIQAAANAXAGIAAgoQgLABgGAFg");
	this.shape_34.setTransform(-38.825,5.05);

	this.shape_35 = new cjs.Shape();
	this.shape_35.graphics.f("#FFFFFF").s().p("AgNArIAAhAIgSAAIAAgSIASAAIAAgdIATAAIAAAdIAVAAIAAASIgVAAIAAA8QAAANAKgBQAHAAAEgDIAFAOQgHAHgOAAQgYAAAAgag");
	this.shape_35.setTransform(-54.175,5.75);

	this.shape_36 = new cjs.Shape();
	this.shape_36.graphics.f("#FFFFFF").s().p("AgjAoQgQgPgBgZQAAgWAQgQQAPgRAWAAQAYAAAOARQAOAQAAAYIAAAFIhUAAQACAOAJAJQALAKANAAQAUAAAMgNIAKANQgQAQgcAAQgWAAgPgQgAAigHQgBgMgHgJQgJgKgQAAQgOAAgJAKQgIAJgBAMIBBAAIAAAAg");
	this.shape_36.setTransform(-63.6,7.075);

	this.shape_37 = new cjs.Shape();
	this.shape_37.graphics.f("#FFFFFF").s().p("AgsA3QgXgVAAgiQAAghAXgWQAVgUAgAAQAkAAAWAdIgSAKQgPgUgZABQgXAAgPAPQgPAQAAAYQAAAYAPARQAQAPAWAAQAVAAAQgOIAAgaIguAAIAAgTIBEAAIAAA1QgYAagjAAQggAAgVgVg");
	this.shape_37.setTransform(-78.075,5.1);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_37},{t:this.shape_36},{t:this.shape_35},{t:this.shape_34},{t:this.shape_33},{t:this.shape_32},{t:this.shape_31},{t:this.shape_30},{t:this.shape_29},{t:this.shape_28},{t:this.shape_27},{t:this.shape_26},{t:this.shape_25},{t:this.shape_24},{t:this.shape_23},{t:this.shape_22},{t:this.shape_21},{t:this.shape_20},{t:this.shape_19},{t:this.shape_18},{t:this.shape_17},{t:this.shape_16},{t:this.shape_15},{t:this.shape_14},{t:this.shape_13},{t:this.shape_12},{t:this.shape_11},{t:this.shape_10},{t:this.shape_9},{t:this.shape_8},{t:this.shape_7},{t:this.shape_6},{t:this.shape_5},{t:this.shape_4},{t:this.shape_3},{t:this.shape_2},{t:this.shape_1},{t:this.shape}]}).wait(1));

	this._renderFirstFrame();

}).prototype = getMCSymbolPrototype(lib.HEAD2, new cjs.Rectangle(-150,-7,300.1,48.5), null);


(lib.Findoutifyouqualify = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#CE0058").s().p("AgfA1IACgNIAHACQAIAAADgIIAEgKIgghNIAQAAIAXA8IAYg8IAQAAIgmBaQgGARgSAAQgFAAgEgBg");
	this.shape.setTransform(84.475,26.95);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("#CE0058").s().p("AgLA2IAAg/IgMAAIAAgNIAMAAIAAgFQAAgMAHgHQAFgHALAAQAHAAAFADIgDALQgDgCgEAAQgLAAAAAOIAAAFIAPAAIAAANIgPAAIAAA/g");
	this.shape_1.setTransform(78.775,23.825);

	this.shape_2 = new cjs.Shape();
	this.shape_2.graphics.f("#CE0058").s().p("AgGA2IAAhMIANAAIAABMgAgGgmQgCgCAAgEQAAgEACgDQADgCADAAQAEAAACACQADADAAAEQAAAEgDACQgCADgEAAQgDAAgDgDg");
	this.shape_2.setTransform(74.425,23.825);

	this.shape_3 = new cjs.Shape();
	this.shape_3.graphics.f("#CE0058").s().p("AgGA1IAAhpIANAAIAABpg");
	this.shape_3.setTransform(70.875,23.925);

	this.shape_4 = new cjs.Shape();
	this.shape_4.graphics.f("#CE0058").s().p("AgYAiQgIgHAAgMQAAgMAIgHQAIgFALAAQAPgBAJAKIAAgMQAAgHgFgFQgFgDgJAAQgMgBgKALIgGgKQANgNARAAQAOAAAIAHQAJAGAAAOIAAA0IgOAAIAAgJQgJAKgPAAQgLAAgIgGgAgNAEQgFAEAAAHQAAAHAFAEQAFAEAIAAQAMABAHgKIAAgMQgHgJgMAAQgIAAgFAEg");
	this.shape_4.setTransform(64.175,25.4);

	this.shape_5 = new cjs.Shape();
	this.shape_5.graphics.f("#CE0058").s().p("AggAPIAAg2IAOAAIAAAxQAAARARAAQAMAAAIgLIAAg3IAOAAIAABNIgOAAIAAgLQgMANgPAAQgYAAAAgZg");
	this.shape_5.setTransform(55.775,25.475);

	this.shape_6 = new cjs.Shape();
	this.shape_6.graphics.f("#CE0058").s().p("AAXA2IAAgoQgKANgPAAQgPAAgJgLQgKgLAAgSQAAgTAKgLQAJgKAPAAQAPAAAKANIAAgMIAOAAIAABqgAgPghQgGAIAAAMQAAAMAGAHQAHAIAJAAQAHAAAGgEQAGgDADgFIAAgfQgDgFgGgDQgGgEgHAAQgJAAgHAIg");
	this.shape_6.setTransform(46.825,26.775);

	this.shape_7 = new cjs.Shape();
	this.shape_7.graphics.f("#CE0058").s().p("AggAPIAAg2IAOAAIAAAxQAAARARAAQAMAAAIgLIAAg3IAOAAIAABNIgOAAIAAgLQgMANgPAAQgYAAAAgZg");
	this.shape_7.setTransform(33.675,25.475);

	this.shape_8 = new cjs.Shape();
	this.shape_8.graphics.f("#CE0058").s().p("AgbAdQgLgMAAgRQAAgQALgMQAKgMARAAQASAAALAMQAKAMAAAQQAAARgKAMQgLAMgSgBQgRABgKgMgAgRgSQgGAHAAALQAAAMAGAHQAHAJAKAAQALAAAHgJQAGgHAAgMQAAgLgGgHQgHgJgLAAQgKAAgHAJg");
	this.shape_8.setTransform(24.925,25.4);

	this.shape_9 = new cjs.Shape();
	this.shape_9.graphics.f("#CE0058").s().p("AgfA1IACgNIAHACQAIAAADgIIAEgKIgghNIAQAAIAXA8IAYg8IAQAAIgmBaQgGARgSAAQgFAAgEgBg");
	this.shape_9.setTransform(16.325,26.95);

	this.shape_10 = new cjs.Shape();
	this.shape_10.graphics.f("#CE0058").s().p("AgLA2IAAg/IgMAAIAAgNIAMAAIAAgFQAAgMAHgHQAFgHALAAQAHAAAFADIgDALQgDgCgEAAQgLAAAAAOIAAAFIAPAAIAAANIgPAAIAAA/g");
	this.shape_10.setTransform(6.225,23.825);

	this.shape_11 = new cjs.Shape();
	this.shape_11.graphics.f("#CE0058").s().p("AgGA2IAAhMIANAAIAABMgAgGgmQgCgCAAgEQAAgEACgDQADgCADAAQAEAAACACQADADAAAEQAAAEgDACQgCADgEAAQgDAAgDgDg");
	this.shape_11.setTransform(1.875,23.825);

	this.shape_12 = new cjs.Shape();
	this.shape_12.graphics.f("#CE0058").s().p("AgJAfIAAguIgNAAIAAgNIANAAIAAgVIANAAIAAAVIAQAAIAAANIgQAAIAAArQAAAJAIAAQAFAAACgDIAEALQgFAFgKAAQgRAAAAgTg");
	this.shape_12.setTransform(71.375,8.425);

	this.shape_13 = new cjs.Shape();
	this.shape_13.graphics.f("#CE0058").s().p("AggAPIAAg2IAOAAIAAAxQAAARARAAQAMAAAIgLIAAg3IAOAAIAABNIgOAAIAAgLQgMANgPAAQgYAAAAgZg");
	this.shape_13.setTransform(64.525,9.475);

	this.shape_14 = new cjs.Shape();
	this.shape_14.graphics.f("#CE0058").s().p("AgbAcQgLgLAAgRQAAgQALgMQAKgMARAAQASAAALAMQAKAMAAAQQAAARgKALQgLAMgSAAQgRAAgKgMgAgRgSQgGAIAAAKQAAALAGAIQAHAIAKAAQALAAAHgIQAGgIAAgLQAAgKgGgIQgHgJgLAAQgKAAgHAJg");
	this.shape_14.setTransform(55.775,9.4);

	this.shape_15 = new cjs.Shape();
	this.shape_15.graphics.f("#CE0058").s().p("AgaArQgKgLAAgSQAAgSAKgLQAJgLAQAAQAOAAAKANIAAgoIAOAAIAABpIgOAAIAAgLQgKANgOAAQgQAAgJgLgAgPgFQgGAGAAANQAAAMAGAIQAHAHAJAAQAHAAAGgDQAGgDADgFIAAggQgDgFgGgDQgGgDgHAAQgJAAgHAIg");
	this.shape_15.setTransform(42.325,8.025);

	this.shape_16 = new cjs.Shape();
	this.shape_16.graphics.f("#CE0058").s().p("AATAnIAAgwQAAgRgRAAQgMAAgIALIAAA2IgOAAIAAhMIAOAAIAAALQAEgFAHgDQAHgEAJgBQAYAAAAAZIAAA1g");
	this.shape_16.setTransform(33.575,9.3);

	this.shape_17 = new cjs.Shape();
	this.shape_17.graphics.f("#CE0058").s().p("AgGA2IAAhMIANAAIAABMgAgGgmQgCgCAAgEQAAgEACgDQADgCADAAQAEAAACACQADADAAAEQAAAEgDACQgCADgEAAQgDAAgDgDg");
	this.shape_17.setTransform(27.425,7.825);

	this.shape_18 = new cjs.Shape();
	this.shape_18.graphics.f("#CE0058").s().p("AgjA1IAAhpIBHAAIAAAOIg3AAIAAAfIA2AAIAAANIg2AAIAAAvg");
	this.shape_18.setTransform(20.575,7.925);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_18},{t:this.shape_17},{t:this.shape_16},{t:this.shape_15},{t:this.shape_14},{t:this.shape_13},{t:this.shape_12},{t:this.shape_11},{t:this.shape_10},{t:this.shape_9},{t:this.shape_8},{t:this.shape_7},{t:this.shape_6},{t:this.shape_5},{t:this.shape_4},{t:this.shape_3},{t:this.shape_2},{t:this.shape_1},{t:this.shape}]}).wait(1));

	this._renderFirstFrame();

}).prototype = getMCSymbolPrototype(lib.Findoutifyouqualify, new cjs.Rectangle(-2,-1,92.7,36), null);


(lib.coahreveps = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// coa-h-rev.eps
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#FFFFFF").s().p("");
	this.shape.setTransform(361.0507,13.7);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("#FFFFFF").s().p("");
	this.shape_1.setTransform(-246.3661,64.3125);

	this.shape_2 = new cjs.Shape();
	this.shape_2.graphics.f("#FFFFFF").s().p("AiJCHQg2g5ABhRQAAhWA3g1QA3g1BXAAQBXAAAvAzQAzA2AABmIkqAAQATBwB6AAQBHAABOgtIAABIQgoAYglAKQgoALg0AAQheAAg6g9gAhDhqQgdAXgIAuIDUAAQgBgsgbgZQgagYgtAAQgvAAgdAYg");
	this.shape_2.setTransform(269.85,97.05);

	this.shape_3 = new cjs.Shape();
	this.shape_3.graphics.f("#FFFFFF").s().p("");
	this.shape_3.setTransform(229.4,106.7);

	this.shape_4 = new cjs.Shape();
	this.shape_4.graphics.f("#FFFFFF").s().p("AA6C7QgQgKgFgTQg3Amg7AAQgtAAgfgcQgcgbAAglQAAgyAvgeQAagQBHgWIBLgYIAAgUQAAgigTgRQgTgQgmAAQhLABg8A9IAAhRQAygzBcAAQBDAAApAeQAtAhAAA9IAADBQAAAUAQAAQAOgBAegTIAAAtQgeARgPAGQgRAGgTAAQgcAAgPgJgAg4AmQgoAXAAAdQAAAXARAQQARAOAaAAQAlAAAlgfIAAhuQhDAVgbAPg");
	this.shape_4.setTransform(191.3,97.05);

	this.shape_5 = new cjs.Shape();
	this.shape_5.graphics.f("#FFFFFF").s().p("AgvDGQgjggAAg1IAAjGIhKAAIAAgIICdiJIAABOICGAAIAABDIiGAAIAADAQAAAeAVAPQAQAMAYAAQArAAA0gcIAABGQgWALgdAHQgfAHgUAAQhCAAgkghg");
	this.shape_5.setTransform(152.55,93.525);

	this.shape_6 = new cjs.Shape();
	this.shape_6.graphics.f("#FFFFFF").s().p("ABeDAIAAjoQAAgqgWgZQgXgZgmAAQgaAAgZANQgYAMgeAeIAAENIhUAAIAAl4IBUAAIAAAyIACAAQAYgbAigPQAhgPAjAAQBAAAAmAlQArAoAABLIAADng");
	this.shape_6.setTransform(116.275,96.675);

	this.shape_7 = new cjs.Shape();
	this.shape_7.graphics.f("#FFFFFF").s().p("AA6C7QgQgKgFgTQg3Amg7AAQgtAAgfgcQgcgbAAglQAAgyAvgeQAagQBHgWIBLgYIAAgUQgBgigTgRQgSgQgnAAQhKABg8A9IAAhRQAygzBcAAQBDAAApAeQAsAhABA9IAADBQAAAUAQAAQAOgBAegTIAAAtQgeARgPAGQgRAGgTAAQgcAAgPgJgAg4AmQgoAXAAAdQAAAXARAQQARAOAaAAQAlAAAlgfIAAhuQhCAVgcAPg");
	this.shape_7.setTransform(75.2,97.05);

	this.shape_8 = new cjs.Shape();
	this.shape_8.graphics.f("#FFFFFF").s().p("AgLDAIjAl/IBcAAIBxDmIBwjmIBaAAIi7F/g");
	this.shape_8.setTransform(33.775,97.425);

	this.shape_9 = new cjs.Shape();
	this.shape_9.graphics.f("#FFFFFF").s().p("AAOD5QheAAg4g2Qg3g1AAhbQgBhRA8g0QA7g1BcAAQArAAAuAPIAAiAIBUAAIAAHxgAhEggQgmAjAAA7QAAB8CCAAIBUAAIAAjvQgpgPglAAQg8AAgmAkg");
	this.shape_9.setTransform(-9.45,91.025);

	this.shape_10 = new cjs.Shape();
	this.shape_10.graphics.f("#FFFFFF").s().p("AC6DyIhJiOIjlAAIhECOIhQAAIDpnjIA2AAIDyHjgAhWAmICnAAIhWitg");
	this.shape_10.setTransform(-54.775,91.7);

	this.shape_11 = new cjs.Shape();
	this.shape_11.graphics.f("#FFFFFF").s().p("ABdDAIAAjoQABgqgXgYQgWgZgmgBQgaAAgYANQgZAMgcAeIAAENIhUAAIAAl4IBUAAIAAAyIABAAQAYgbAhgQQAigPAiAAQA/AAAmAmQAqApAABJIAADog");
	this.shape_11.setTransform(331.75,28.15);

	this.shape_12 = new cjs.Shape();
	this.shape_12.graphics.f("#FFFFFF").s().p("AihCMQg/g5AAhWQAAhRBAg4QBAg3BeAAQBiAABBA2QBAA3AABTQAABZg/A3Qg/A3hlAAQhgAAg/g4gAhkhcQgnAjAAA2QAAA7AmAlQAlAkA9AAQBCAAAmgjQAngkgBg8QAAg3gogjQgogkg+AAQg7AAgmAkg");
	this.shape_12.setTransform(285.3,28.525);

	this.shape_13 = new cjs.Shape();
	this.shape_13.graphics.f("#FFFFFF").s().p("");
	this.shape_13.setTransform(241.575,38.2);

	this.shape_14 = new cjs.Shape();
	this.shape_14.graphics.f("#FFFFFF").s().p("AiICHQg1g5AAhSQAAhVA3g1QA3g1BWAAQBWAAAvAzQAyA2AABlIknAAQATByB5AAQBGAABNgtIAABIQgnAXglAKQgoALg0AAQhdAAg5g9gAhChqQgdAYgJAtIDTAAQgCgrgagZQgagZgsAAQguAAgdAYg");
	this.shape_14.setTransform(199.925,28.525);

	this.shape_15 = new cjs.Shape();
	this.shape_15.graphics.f("#FFFFFF").s().p("AiZDAIAAl4IBUAAIAABXIACAAQA8hfA8AAQAxAAA0AsIgsBBQgTgQgXgLQgVgKgPAAQgqAAgdAiQgeAhAAAxIAADEg");
	this.shape_15.setTransform(166.55,28.15);

	this.shape_16 = new cjs.Shape();
	this.shape_16.graphics.f("#FFFFFF").s().p("AjMCnQhLhEAAhjQAAhjBMhEQBQhIB7AAQB9AABPBIQBMBEAABjQAABjhLBEQhQBJh+AAQh8AAhPhJgAiQh/Qg6AzAABMQAABNA6AzQA5AzBXgBQBXABA6gzQA6gzAAhNQAAhMg6gzQg6gyhXAAQhXAAg5Ayg");
	this.shape_16.setTransform(116.7,24);

	this.shape_17 = new cjs.Shape();
	this.shape_17.graphics.f("#FFFFFF").s().p("AiICHQg1g5AAhSQAAhVA3g1QA3g1BWAAQBWAAAvAzQAyA2AABlIknAAQATByB5AAQBGAABNgtIAABIQgnAXglAKQgnALg1AAQhdAAg5g9gAhChqQgdAYgJAtIDTAAQgCgrgagZQgagZgsAAQguAAgdAYg");
	this.shape_17.setTransform(62.775,28.525);

	this.shape_18 = new cjs.Shape();
	this.shape_18.graphics.f("#FFFFFF").s().p("AiZDAIAAl4IBUAAIAABXIACAAQA8hfA8AAQAxAAAzAsIgsBBQgSgQgWgLQgWgKgPAAQgqAAgdAiQgeAhAAAxIAADEg");
	this.shape_18.setTransform(30,28.15);

	this.shape_19 = new cjs.Shape();
	this.shape_19.graphics.f("#FFFFFF").s().p("AA5C7QgQgJgFgUQg1Amg7AAQgtAAgegcQgcgaAAgmQAAgyAugeQAagRBHgVIBJgYIAAgUQAAgjgTgQQgSgQgmAAQhLAAg6A/IAAhSQAxgzBbAAQBCAAApAeQAsAhAAA9IAADBQAAAUAQAAQAPAAAdgUIAAAtQgdARgQAGQgQAGgTAAQgcAAgPgJgAg3AmQgoAWAAAeQAAAYARAPQARAOAaAAQAkAAAkgfIAAhuQhCAVgaAPg");
	this.shape_19.setTransform(-9.325,28.525);

	this.shape_20 = new cjs.Shape();
	this.shape_20.graphics.f("#FFFFFF").s().p("AinCgQhDhDAAhWQAAhqBRhGQBRhGB7AAQBLAABoAmIAABIQhegwhWAAQhZAAg6AzQg8AzAABMQAABPA7AxQA6AxBdAAQBcAABag0IAABJQgqAUgpAKQgxALg4AAQiGAAhQhQg");
	this.shape_20.setTransform(-56.9,24.45);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_20},{t:this.shape_19},{t:this.shape_18},{t:this.shape_17},{t:this.shape_16},{t:this.shape_15},{t:this.shape_14},{t:this.shape_13},{t:this.shape_12},{t:this.shape_11},{t:this.shape_10},{t:this.shape_9},{t:this.shape_8},{t:this.shape_7},{t:this.shape_6},{t:this.shape_5},{t:this.shape_4},{t:this.shape_3},{t:this.shape_2},{t:this.shape_1},{t:this.shape}]}).wait(1));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-367.1,0,734.3,135.2);


(lib.BKGDrubine = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#CE0058").s().p("A3bTiMAAAgnDMAu3AAAMAAAAnDg");

	this.timeline.addTween(cjs.Tween.get(this.shape).wait(1));

	this._renderFirstFrame();

}).prototype = getMCSymbolPrototype(lib.BKGDrubine, new cjs.Rectangle(-150,-125,300,250), null);


(lib.LOGO = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1
	this.instance = new lib.coahreveps("synched",0);
	this.instance.setTransform(116.45,35.7,0.28,0.28,0,0,0,415.9,115.9);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	this._renderFirstFrame();

}).prototype = getMCSymbolPrototype(lib.LOGO, new cjs.Rectangle(-102.7,3.3,205.60000000000002,37.800000000000004), null);


(lib.CTA_button1svg = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// CTA_button1.svg
	this.instance = new lib.Findoutifyouqualify();
	this.instance.setTransform(50.6,21.6,1,1,0,0,0,44.4,16.9);

	this.shape = new cjs.Shape();
	this.shape.graphics.f("#FFFFFF").s().p("AmZDcQgqAAgdgdQgdgeAAgpIAAjvQAAgqAdgcQAdgeAqAAIMzAAQApAAAeAeQAdAdAAApIAADvQAAApgdAeQgeAdgpAAg");
	this.shape.setTransform(51,22);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape},{t:this.instance}]}).wait(1));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,102,44);


(lib.CTA_mobile = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_4
	this.instance = new lib.CTA_button1svg("synched",0);
	this.instance.setTransform(0,-2,0.9,0.9,0,0,0,51,21.9);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	this._renderFirstFrame();

}).prototype = getMCSymbolPrototype(lib.CTA_mobile, new cjs.Rectangle(-45.9,-21.7,91.8,39.599999999999994), null);


// stage content:
(lib.COAEPdental320x50 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {"loop point":13,"if/then":232};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	this.actionFrames = [232,243];
	// timeline functions:
	this.frame_232 = function() {
		if(!this.alreadyExecuted){
		this.alreadyExecuted=true;
		this.loopNum=1;
		} else {
		this.loopNum++;
		if(this.loopNum==2){
		this.stop();
		}
		}
	}
	this.frame_243 = function() {
		//if(!this.alreadyExecuted){
		//this.alreadyExecuted=true;
		//this.loopNum=1;
		//} else {
		//this.loopNum++;
		//if(this.loopNum==3){
		this.gotoAndPlay(14);
		//}
		//}
	}

	// actions tween:
	this.timeline.addTween(cjs.Tween.get(this).wait(232).call(this.frame_232).wait(11).call(this.frame_243).wait(1));

	// border
	this.shape = new cjs.Shape();
	this.shape.graphics.f().s("#999999").ss(1,2,0,3).p("A46j0MAx1AAAIAAHpMgx1AAAg");
	this.shape.setTransform(160,25);

	this.timeline.addTween(cjs.Tween.get(this.shape).wait(244));

	// HEAD2
	this.instance = new lib.HEAD2();
	this.instance.setTransform(160,72,1,1,0,0,0,0,24);
	this.instance.alpha = 0;

	this.timeline.addTween(cjs.Tween.get(this.instance).to({y:32,alpha:1},13,cjs.Ease.quadOut).wait(97).to({alpha:0},7).to({_off:true},1).wait(126));

	// LOGO
	this.instance_1 = new lib.LOGO();
	this.instance_1.setTransform(112.95,26.85,0.92,0.92,0,0,0,8,21.7);
	this.instance_1._off = true;

	this.timeline.addTween(cjs.Tween.get(this.instance_1).wait(118).to({_off:false},0).wait(115).to({alpha:0},8).wait(3));

	// CTA
	this.instance_2 = new lib.CTA_mobile();
	this.instance_2.setTransform(262,27);
	this.instance_2._off = true;

	this.timeline.addTween(cjs.Tween.get(this.instance_2).wait(118).to({_off:false},0).wait(115).to({alpha:0},8).wait(3));

	// BKGD-rubine
	this.instance_3 = new lib.BKGDrubine();
	this.instance_3.setTransform(160.25,49.2,2.4267,0.36,0,0,0,0.1,67.2);

	this.timeline.addTween(cjs.Tween.get(this.instance_3).wait(244));

	this._renderFirstFrame();

}).prototype = p = new lib.AnMovieClip();
p.nominalBounds = new cjs.Rectangle(-44,5,568,84.5);
// library properties:
lib.properties = {
	id: '0957C5EC3B874C629166840920B09671',
	width: 320,
	height: 50,
	fps: 36,
	color: "#999999",
	opacity: 1.00,
	manifest: [],
	preloads: []
};



// bootstrap callback support:

(lib.Stage = function(canvas) {
	createjs.Stage.call(this, canvas);
}).prototype = p = new createjs.Stage();

p.setAutoPlay = function(autoPlay) {
	this.tickEnabled = autoPlay;
}
p.play = function() { this.tickEnabled = true; this.getChildAt(0).gotoAndPlay(this.getTimelinePosition()) }
p.stop = function(ms) { if(ms) this.seek(ms); this.tickEnabled = false; }
p.seek = function(ms) { this.tickEnabled = true; this.getChildAt(0).gotoAndStop(lib.properties.fps * ms / 1000); }
p.getDuration = function() { return this.getChildAt(0).totalFrames / lib.properties.fps * 1000; }

p.getTimelinePosition = function() { return this.getChildAt(0).currentFrame / lib.properties.fps * 1000; }

an.bootcompsLoaded = an.bootcompsLoaded || [];
if(!an.bootstrapListeners) {
	an.bootstrapListeners=[];
}

an.bootstrapCallback=function(fnCallback) {
	an.bootstrapListeners.push(fnCallback);
	if(an.bootcompsLoaded.length > 0) {
		for(var i=0; i<an.bootcompsLoaded.length; ++i) {
			fnCallback(an.bootcompsLoaded[i]);
		}
	}
};

an.compositions = an.compositions || {};
an.compositions['0957C5EC3B874C629166840920B09671'] = {
	getStage: function() { return exportRoot.stage; },
	getLibrary: function() { return lib; },
	getSpriteSheet: function() { return ss; },
	getImages: function() { return img; }
};

an.compositionLoaded = function(id) {
	an.bootcompsLoaded.push(id);
	for(var j=0; j<an.bootstrapListeners.length; j++) {
		an.bootstrapListeners[j](id);
	}
}

an.getComposition = function(id) {
	return an.compositions[id];
}


an.makeResponsive = function(isResp, respDim, isScale, scaleType, domContainers) {		
	var lastW, lastH, lastS=1;		
	window.addEventListener('resize', resizeCanvas);		
	resizeCanvas();		
	function resizeCanvas() {			
		var w = lib.properties.width, h = lib.properties.height;			
		var iw = window.innerWidth, ih=window.innerHeight;			
		var pRatio = window.devicePixelRatio || 1, xRatio=iw/w, yRatio=ih/h, sRatio=1;			
		if(isResp) {                
			if((respDim=='width'&&lastW==iw) || (respDim=='height'&&lastH==ih)) {                    
				sRatio = lastS;                
			}				
			else if(!isScale) {					
				if(iw<w || ih<h)						
					sRatio = Math.min(xRatio, yRatio);				
			}				
			else if(scaleType==1) {					
				sRatio = Math.min(xRatio, yRatio);				
			}				
			else if(scaleType==2) {					
				sRatio = Math.max(xRatio, yRatio);				
			}			
		}
		domContainers[0].width = w * pRatio * sRatio;			
		domContainers[0].height = h * pRatio * sRatio;
		domContainers.forEach(function(container) {				
			container.style.width = w * sRatio + 'px';				
			container.style.height = h * sRatio + 'px';			
		});
		stage.scaleX = pRatio*sRatio;			
		stage.scaleY = pRatio*sRatio;
		lastW = iw; lastH = ih; lastS = sRatio;            
		stage.tickOnUpdate = false;            
		stage.update();            
		stage.tickOnUpdate = true;		
	}
}
an.handleSoundStreamOnTick = function(event) {
	if(!event.paused){
		var stageChild = stage.getChildAt(0);
		if(!stageChild.paused || stageChild.ignorePause){
			stageChild.syncStreamSounds();
		}
	}
}
an.handleFilterCache = function(event) {
	if(!event.paused){
		var target = event.target;
		if(target){
			if(target.filterCacheList){
				for(var index = 0; index < target.filterCacheList.length ; index++){
					var cacheInst = target.filterCacheList[index];
					if((cacheInst.startFrame <= target.currentFrame) && (target.currentFrame <= cacheInst.endFrame)){
						cacheInst.instance.cache(cacheInst.x, cacheInst.y, cacheInst.w, cacheInst.h);
					}
				}
			}
		}
	}
}


})(createjs = createjs||{}, AdobeAn = AdobeAn||{});
var createjs, AdobeAn;